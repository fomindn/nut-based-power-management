...
ssh_available() {
    local host="$1"
    "$TIMEOUT_BIN" "$SSH_TIMEOUT" "$SSH_BIN" -o BatchMode=yes -o ConnectTimeout=3 "$host" true >/dev/null 2>&1
}
...
ssh_graceful_shutdown() {
    ...
    if ssh_available "$host"; then
        ...
        "$SSH_BIN" "$host" "shutdown -h now" || return 1
    else
